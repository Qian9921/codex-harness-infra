...
交付由本地 `[delivery]` 明确：`local_only`（默认）、`pull_request`、或 `merge_if_ready`，并列出授权仓库。当前用户对指定仓库的明确 PR 请求可用请求范围 effective config 覆盖 standing `local_only`，不改持久偏好、不重问。安装或配置凭据不是发布授权。
