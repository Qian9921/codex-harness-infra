...
实现、测试、数据运行、恢复和已授权 Git 工作默认由 `$grok-execution` 直接调用外部 Grok 4.6 Build，reasoning effort 固定为 `low`。`run`/`resume` 默认无墙钟超时。Grok 任务须覆盖实现/测试/修复，并返回精简的文件、行为、证据与未决项；Primary 做针对性独立核验，不重复整份实现。所有 Codex Grok `run`/`resume` 由单独 spawn 的通用 Luna-low native subagent 只监督生命周期与 receipt，不得编辑且不是 `v23_executor`。只有可验证的 `QUOTA_EXHAUSTED` / `grok_quota_exhausted` receipt 才允许 `v23_executor` fallback。超时、认证、网络、bridge、模型身份或 receipt 错误必须修复 Grok 或报告 `GROK_EXECUTION_BLOCKED`。专用 PGID/信号清理见已安装 Grok skill `references/grok-process-lifecycle.md`。递归内容搜索默认使用已安装 `bin/bounded-search.py`（Harness 默认，不是 OS sandbox）；15 秒超时；超时或不完整须收窄后重试，不得当成无匹配。已知单文件读取可直接进行。内部 poll 不面向用户；用户可见更新仅限开始、有意义状态变化、完成或失败。空的结构化 `request_user_input` 答案视为未回答：任务保持暂停，resume 时原问重现，不得写入或推断默认值。reviewer 仍独立审查。
