...
"""
        ...
        for label, policy, spec, distinctive in cases:
            ...
            self.assertIn("check the owner repository CodeGraph index", instructions)
            self.assertIn("BEFORE code exploration", instructions)
            self.assertIn("focused structural query", instructions)
            self.assertIn("current tree rather than commit metadata alone", instructions)
            self.assertIn("finite verified supported set", instructions)
            self.assertIn("preserves exit status and diagnostics", instructions)
            self.assertIn("never dismiss it as not needed", instructions)
