...
Discussion normally stays read-only. Repository publication follows the installed `[delivery]` policy (`local_only`, `pull_request`, or `merge_if_ready`) and explicit `repositories`. Credentials and install are not publication permission. Current user instructions override those defaults. Deletion, production release, credentials, and other irreversible external actions require separate confirmation; PR authorization does not imply them.
