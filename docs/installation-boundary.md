...
Local model mapping is environment data, not portable policy. Shared templates stay model-agnostic placeholders. Do not copy a current native version slug into standing principles. Record freshness at install, update, or actual use; do not auto-upgrade names. Machine-specific preferences, including an existing Martin mapping, are applied only when that operator copies them into a local config at install.
