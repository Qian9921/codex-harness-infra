...
Installation is deliberately narrow. It makes the portable policy and logical role templates available to Codex, installs the delivery and Grok/Pi-execution Skills plus the owned CodeGraph routing Skill, and configures the small local adapters needed for runtime-state injection, opt-in bounded search, authorized GitHub delivery, and the explicitly loaded Pi tool-routing extension.
...
The project does not manage or automatically start external tools that happen to be available on the machine. The sole exception is one UserPromptSubmit hook, installed as a marked inline config block, which injects installed instructions and a bounded live runtime-state block. The default hook budget covers installed file reads and Doctor-subset integrity (no daemon JSON). Explicit Doctor `--probe-daemons` still uses the measured `codex app-server daemon version` floor (~9s, 12s timeout). Explicit `--probe-tools` still uses the measured Semble (~29s floor, 36s timeout) and CodeGraph budgets. Install and migration report a bounded installed-version and `codegraph upgrade --check` result for configured tools; explicit maintenance can rerun it with `--probe-updates`. All of these report offline or unsupported checks honestly and never upgrade a tool. It does not use a Stop hook, daemon, background service, or project-tracked index. CodeGraph's cache exclusion is Git-local and marked before a V23-created cache is initialized.
...
After installation, use Codex's hook browser once to review and trust the V23 UserPromptSubmit command. Codex requires explicit trust for non-managed command hooks; the installer never bypasses that protection. The doctor command requires canonical `${CODEX_HOME}/AGENTS.md` for managed-block checks, reports a non-empty `AGENTS.override.md` as `global_override_absent=false`, and names the effective Codex source in `active_global_instruction` (the non-empty override when present, otherwise `AGENTS.md`, matching official skip-empty behavior). It also reports local marked blocks, prompt hook, logical role files, the owned Pi enforcement extension, the owned CodeGraph routing skill, real bounded tool probes when requested, and configured GitHub adapter. Doctor does not recursively invoke the full UserPromptSubmit bootstrap when it is itself asked only for a summary. The task hook's live runtime-state block treats `${CODEX_HOME}/harness/v23-state/install.json` and instruction/config integrity as authority; daemon probes are explicit. Memory of prior tasks is historical only. An explicit Doctor CodeGraph probe can create the V23-marked Git-local cache exclusion and index. Its local terminal output may name local files for repair, but it never prints credentials and that output must not be committed. Start a new Codex task after changed instructions or hook configuration are installed.

Local model mapping is environment data, not portable policy. Shared templates stay model-agnostic placeholders. Do not copy a current native version slug into standing principles. Record freshness at install, update, or actual use; do not auto-upgrade names. Machine-specific preferences, including an existing operator mapping, are applied only when that operator copies them into a local config at install.
