...
The project does not manage or automatically start external tools that happen to be available on the machine. The sole exception is one UserPromptSubmit hook, installed as a marked inline config block, which injects installed instructions and a bounded live runtime-state block. The hook budget covers live daemon-version JSON (12s) plus process overhead; it no longer runs a mandatory CodeGraph/Semble/RTK sequence on every prompt. Explicit Doctor and `probe_tools` calls still use the measured Semble (~29s floor, 36s timeout) and CodeGraph budgets. It does not use a Stop hook, daemon, background service, or project-tracked index. CodeGraph's cache exclusion is Git-local and marked before a V23-created cache is initialized.
...
After installation, use Codex's hook browser once to review and trust the V23 UserPromptSubmit command. Codex requires explicit trust for non-managed command hooks; the installer never bypasses that protection. The doctor command requires canonical `${CODEX_HOME}/AGENTS.md` for managed-block checks, reports a non-empty `AGENTS.override.md` as `global_override_absent=false`, and names the effective Codex source in `active_global_instruction` (the non-empty override when present, otherwise `AGENTS.md`, matching official skip-empty behavior). It also reports local marked blocks, prompt hook, logical role files, real bounded tool probes when requested, and configured GitHub adapter. Doctor does not recursively invoke the full UserPromptSubmit bootstrap when it is itself asked only for a summary. The task hook's live runtime-state block treats `${CODEX_HOME}/harness/v23-state/install.json` and live daemon probes as authority; memory of prior tasks is historical only. An explicit Doctor CodeGraph probe can create the V23-marked Git-local cache exclusion and index. Its local terminal output may name local files for repair, but it never prints credentials and that output must not be committed. Start a new Codex task after changed instructions or hook configuration are installed.

Recommended local model mapping (environment data, not portable templates): primary `gpt-6-astra` at high effort, independent reviewer `gpt-5.6-sol`, quota-only native fallback Luna at low effort. Shared templates stay model-agnostic placeholders.
