...
The project does not manage or automatically start external tools that happen to be available on the machine. The sole exception is one UserPromptSubmit hook, installed as a marked inline config block, which runs a bounded CodeGraph, Semble, and RTK bootstrap for each new user task. Each operation has a small fixed budget whose worst-case sequence stays inside the Hook budget (Git discovery 4s×2, CodeGraph 14s×4, Semble 36s, RTK 8s, live daemon-version JSON 12s, plus 10s process overhead → 130s), so a stalled CodeGraph probe still leaves time for Semble, RTK, and the live runtime-state probes. The Semble and live-probe budgets are sized to measured local floors (~29s semantic search, ~9s `codex app-server daemon version`) rather than optimistic defaults. It does not use a Stop hook, daemon, background service, or project-tracked index. CodeGraph's cache exclusion is Git-local and marked before a V23-created cache is initialized.
