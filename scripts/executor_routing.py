...
TOOL_SELECTION_GUIDANCE = (
    "Tool obligations use explicit triggers; this is never a batch probe of "
    "every task. Code investigation or change: check the owner repository "
    "CodeGraph index (`codegraph status --json <repo>`) BEFORE code exploration, "
    "and cross-check the cited current source. Cross-file "
    ...
    "`rtk pytest <args>` compact test summaries); the explicitly loaded owned "
    "Pi extension routes bare `pytest` through `rtk pytest` by default; "
    "`python -m pytest`, `pytest3`, and explicit interpreter or pytest paths "
    "stay raw because that route does not prove it preserves the selected "
    "executable. Exact JSON, porcelain, "
    "unified diffs, and necessary raw diagnostics are explicit raw exceptions, "
    "unsupported compound shell syntax is never silently rewritten, an absent "
    "or failed rtk falls back to the explicit raw command, and a routed failure "
    "preserves exit status and diagnostics. Tool absent, "
    ...
    "actually invoked when they fit, and missing or failed tools fall back to "
    "baseline. This is an obligation for the listed triggers, not a classifier "
    "or gate."
)
