...
an authorized writable owner repo. A reported zero pending count is not proof
of freshness: writable work syncs before relying on the index, while read-only
...
an absent rtk falls back to the explicit raw command. Tool absence, real
failure, or a read-only missing index: explicit
